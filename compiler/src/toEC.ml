    ...
    | Cfor (i, (d,e1,e2), c) ->
      ...
      let init, ec_e2 =
        ...
        | _ -> 
          ...
          let init = ec_asgn aux (toec_expr env e2) in
          let ec_e2 = ec_ident aux in
          [init], ec_e2 in
      let ec_i = ec_vars env (L.unloc i) in
      let init  = init @ [ec_asgn ec_i (toec_expr env e1)] in
      let ec_i1, ec_i2 =
        if d = UpTo then ec_ident ec_i , ec_e2
        else ec_e2, ec_ident ec_i in
      let i_upd_op = Infix (if d = UpTo then "+" else "-") in
      let i_upd = [ec_asgn ec_i (Eop2 (i_upd_op, ec_ident ec_i, Econst (Z.of_int 1)))] in
